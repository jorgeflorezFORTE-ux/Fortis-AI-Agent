...
function checkDuplicateUpload(accountId, filename, csvContent) {
  const db = getDb();
  const byName = db.prepare('SELECT id, source_name, total_transactions, uploaded_at FROM personal_uploads WHERE account_id = ? AND filename = ?').get(accountId, filename);
...
function deduplicateTransactions(newTxns, companyId) {
  const db = getDb();
  ...
  const existing = db.prepare('SELECT date, amount, description FROM transactions WHERE company_id = ? AND date >= ? AND date <= ?').all(companyId, minDate, maxDate);
  const set = new Set();
...
module.exports = { checkDuplicateUpload: checkDuplicateUpload, deduplicateTransactions: deduplicateTransactions, hashContent: hashContent };
