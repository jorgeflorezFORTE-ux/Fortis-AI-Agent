...
export default async function handler(req, res) {
  ...
  if (!force) { const c = getLatestAnalysis(cid || 'all', period, 'financial'); if (c && Date.now() - new Date(c.created_at).getTime() < 86400000) return res.json({ ...c.result, cached: true }); }
  try {
    const d = getDashboardData({ companyId: cid, year: parseInt(year) });
    const txns = getTransactions({ companyId: cid, startDate: year + '-01-01', endDate: year + '-12-31', type: 'expense', limit: 100 });
    ...
    saveAnalysis(cid || 'all', period, 'financial', a);
