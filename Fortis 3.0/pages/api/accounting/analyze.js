...
export default async function handler(req, res) {
  ...
  try {
    const recon = (!company || company === 'all') ? reconcileAll({ year: parseInt(year), month: parseInt(month) }) : reconcileMonth({ companyId: company, year: parseInt(year), month: parseInt(month) });
