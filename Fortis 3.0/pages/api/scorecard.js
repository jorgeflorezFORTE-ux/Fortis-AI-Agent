...
export default async function handler(req, res) {
  ...
  try {
    const biz = await getDashboardData({ companyId: null, year: parseInt(year) });
    const global = await getGlobalSummary(parseInt(year));
    const recon = await reconcileAll({ year: parseInt(year), month: parseInt(month) });
    const freedom = await getFreedomSummary();
