...
export function extractPLData(plReport) {
  ...
  plReport.Rows.Row.forEach(section => {
    const name = (section.Summary?.ColData?.[0]?.value || section.group || '').toLowerCase();
    const rawVal = parseFloat(section.Summary?.ColData?.[1]?.value || 0);
    const val = isNaN(rawVal) ? 0 : rawVal;

    // Income / Revenue sections
    if (
      name.includes('income') ||
      name.includes('revenue') ||
      name === 'grossprofit' ||
      section.group === 'GrossProfit' ||
      section.group === 'Income'
    ) {
      if (name.includes('gross') || name.includes('total for income') || name === 'income') {
        income = Math.abs(val);
      }
    }
    ...
    if (
      ...
      if (name.includes('total') || name === 'expenses') {
    ...
    if (
      name.includes('net') ||
      section.group === 'NetIncome' ||
      section.group === 'NetOperatingIncome'
    ) {
      net = val;
    }
  });

  // If we got net but not income/expenses separately, derive them
  if (income === 0 && expenses === 0 && net !== 0) {
    // Try alternate parsing — walk all rows
    plReport.Rows.Row.forEach(section => {
      const group = section.group || '';
      const val = Math.abs(parseFloat(section.Summary?.ColData?.[1]?.value || 0));
      if (!isNaN(val)) {
        if (group === 'Income' || group === 'GrossProfit') income = val;
        if (group === 'Expenses' || group === 'COGS') expenses = val;
      }
    });
  }

  // Final fallback: if still zero, sum all data rows
  if (income === 0) {
    plReport.Rows.Row.forEach(section => {
      if (!section.Rows?.Row) return;
      const name = (section.Summary?.ColData?.[0]?.value || '').toLowerCase();
      const val = parseFloat(section.Summary?.ColData?.[1]?.value || 0);
      if (isNaN(val)) return;
      if (name.includes('income') || name.includes('revenue')) income = Math.abs(val);
      else if (name.includes('expense') || name.includes('cost')) expenses = Math.abs(val);
    });
  }
